...
def SendView(item_id):
    ...
    try:
        ...
        try:
            ...
            TotalFailedReq += 1
            Title(f"SenkoBotSHares CPU Thread :{str(active_count()-1)} | Hit :{TotalSendedShare}  Fail :{TotalFailedReq}")
    except:
        pass
...
if (__name__ == "__main__"):
    Clear()
    itemID       = Write.Input("Tiktok Video Link (PC Only Link) > ", Colors.red_to_purple, interval=0.0001)
    amount       = Write.Input("Amount (1 - infinity) > ", Colors.red_to_purple, interval=0.0001)
    NThread      = Write.Input("CPU Thread (if dont sure go back to github!) > ", Colors.red_to_purple, interval=0.0001)
